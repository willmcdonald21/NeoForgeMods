...
public record VillagerPathData(
    ...
    public static final VillagerPathData EMPTY =
            new VillagerPathData(Optional.empty(), Optional.empty(), List.of(), Optional.empty(), 0L, Optional.empty(), 0L);
    ...
    public static final Codec<VillagerPathData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            UUIDUtil.CODEC.optionalFieldOf("zoneId").forGetter(VillagerPathData::zoneId),
    ...
    public static VillagerPathData assigned(UUID pathId) {
        return new VillagerPathData(Optional.of(pathId), Optional.empty(), List.of(), Optional.empty(), 0L, Optional.empty(), 0L);
    }

    public VillagerPathData travelingTo(BlockPos goal, Optional<UUID> zoneId, List<BlockPos> route) {
        return new VillagerPathData(pathId, Optional.of(goal), route, zoneId, 0L, Optional.empty(), 0L);
    }

    public VillagerPathData advancedRoute() {
        List<BlockPos> remaining = route.isEmpty() ? route : route.subList(1, route.size());
        return new VillagerPathData(pathId, currentGoal, List.copyOf(remaining), zoneId, busyUntil, wanderTarget, wanderPauseUntil);
    }

    public VillagerPathData arrivedAt(long busyUntilTime) {
        return new VillagerPathData(pathId, currentGoal, List.of(), zoneId, busyUntilTime, Optional.empty(), 0L);
    }

    public VillagerPathData wanderingTowards(BlockPos target) {
        return new VillagerPathData(pathId, currentGoal, route, zoneId, busyUntil, Optional.of(target), 0L);
    }

    public VillagerPathData pausingUntil(long gameTime) {
        return new VillagerPathData(pathId, currentGoal, route, zoneId, busyUntil, wanderTarget, gameTime);
    }
}
